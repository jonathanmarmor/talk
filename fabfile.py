...
INSTALL_HOME = '/opt'
APP_DIR = os.path.join(INSTALL_HOME, CONF.APP_NAME)
REPO = 'git@github.com:{}/{}.git'.format(CONF.GITHUB_ORGANIZATION, CONF.APP_NAME)
...
def update_repo():
    ...
    put(os.path.join(THIS_DIR, 'config_local.py'), remote_path='/opt/talk/config_local.py', use_sudo=True)
...
def configure_supervisor():
    ...
    with cd('/etc/supervisor/conf.d'):
        put('conf/supervisor.conf', '{}.conf'.format(CONF.APP_NAME), use_sudo=True)
...
def configure_nginx():
    ...
    with cd('/etc/nginx/sites-enabled'):
        put('conf/nginx.conf', '{}.conf'.format(CONF.APP_NAME), use_sudo=True)
...
def restart_supervisor():
    ...
    sudo('supervisorctl restart {}'.format(CONF.APP_NAME))
